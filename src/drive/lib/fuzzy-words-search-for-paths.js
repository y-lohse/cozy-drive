...
const forDebugPackage = function() {
  ...
  const _filterAndScore = function(listItems, words) {
    const suggestions = []
    // eslint-disable-next-line
    itemLoop: for (let item of listItems) {
      let itemScore = 0
      for (let w of words) {
  ...
  const _isAugmentingCurrentQuery = function(query) {
    ...
    priorizedWords = _sortQuerybyLength(priorizedWords).concat(
      _sortQuerybyLength(isFromPreviousQuery)
    )
    previousQuery = query
  ...
  return {
    init: newItemsList => {
      ...
      for (let file of list) {
        ...
        file.pathArray = removeDiacritics(
